...
fn compile_and_render_shader(
    ...
    // Force WGPU renderer usage - NO CPU FALLBACK ALLOWED
    let mut renderer_guard = global_renderer.renderer.lock().unwrap();
    let renderer = renderer_guard.as_mut().ok_or_else(|| {
        "WGPU renderer not initialized - GPU rendering is required".to_string()
    })?;
    
    // Use the real WGPU renderer exclusively
    ...
    match renderer.render_frame(wgsl_code, &params, None) {
        ...
        Err(e) => {
            // CRITICAL: Panic on WGPU failure - NO FALLBACK ALLOWED
            panic!("WGPU renderer failed: {}. GPU rendering is mandatory.", e);
        }
    }
}