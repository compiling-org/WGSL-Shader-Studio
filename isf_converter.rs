...
impl IsfToWgslConverter {
    ...
    pub fn convert_fragment_shader(&self, isf_json: &str, isf_code: &str) -> Result<String> {
        let mut wgsl_code = String::new();
        
        // Generate uniform buffer
        let uniform_buffer = self.generate_uniform_buffer(isf_json)?;
    ...
    fn generate_uniform_buffer(&self, isf_json: &str) -> Result<String> {
        let mut buffer_code = String::new();
        buffer_code.push_str("struct Uniforms {\n");
        
        // Parse inputs from JSON
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(isf_json) {
            if let Some(inputs) = json.get("INPUTS").and_then(|v| v.as_array()) {
                for input in inputs {
                    if let (Some(name), Some(input_type)) = (
                        input.get("NAME").and_then(|v| v.as_str()),
                        input.get("TYPE").and_then(|v| v.as_str())
                    ) {
                        let field_type = self.map_isf_type_to_wgsl(input_type)?;
                        let field_name = self.sanitize_identifier(name);
