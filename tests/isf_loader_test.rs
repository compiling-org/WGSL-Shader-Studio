...
fn test_load_resolume_isf_shaders() {
    ...
    match load_resolume_isf_shaders() {
        Ok(shaders) => {
            println!(
                "Successfully loaded {} ISF shaders from Magic directories",
                shaders.len()
            );
            // Verify that we have some shaders loaded
            assert!(
                shaders.len() >= 0,
                "Should have non-negative number of shaders"
            );

            // If we have shaders, verify their structure
            for shader in &shaders {
                println!(
                    "Loaded shader: {} with {} inputs and {} outputs",
                    shader.name,
                    shader.inputs.len(),
                    shader.outputs.len()
                );
                ...
                assert!(
                    shader.outputs.len() > 0,
                    "Shader should have at least one output"
                );
            }
        }
        Err(e) => {
...
fn test_get_shader_metadata() {
    ...
    assert_eq!(
        ...
        Some("A shader with metadata".to_string())
    );
