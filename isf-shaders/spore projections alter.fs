/*
{
    "DESCRIPTION": "A synthesis of two shaders, combining 3D raymarching with the logic of a 2D fractal.",
...
// The fixed scene function that correctly uses the `baseShape`
float scene(vec3 p, float t, float geo, float chaos, float chaosMix, float LiminalMix, float liminalScale) {
    ...
    // This is the fix: we now use the `baseShape` variable, which is correctly chosen by the slider.
    float finalShape = mix(baseShape, liminalFractal(p, t, liminalScale), LiminalMix);
    
    return mix(finalShape, shapeChaos(p, chaos), chaosMix);
}
