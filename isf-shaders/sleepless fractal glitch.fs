/*{
    "DESCRIPTION": "Base psychedelic fractal merged with reference-style transforms, geometry modes, chaos controls, and texture blending, preserving original animation when TransformMix=0.",
    ...
    "INPUTS": [
        ...
        { "NAME": "TransformMode", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0, "MAX": 5 },
        { "NAME": "TransformMix", "TYPE": "float", "DEFAULT": 0.5, "MIN": 0.0, "MAX": 1.0 },
        { "NAME": "GeometryType", "TYPE": "float", "DEFAULT": 2.0, "MIN": 0, "MAX": 6 },
        { "NAME": "ChaosIntensity", "TYPE": "float", "DEFAULT": 0.5, "MIN": 0.0, "MAX": 2.0 },
        ...
        { "NAME": "StepCount", "TYPE": "float", "DEFAULT": 12, "MIN": 1, "MAX": 60 },
...
float scene(vec3 p, int geo, float chaos, float mixAmt) {
    ...
    else if (geo == 1) { vec2 q = vec2(length(p.xz)-1.0, p.y); base = length(q) - 0.3; }
    else if (geo == 2) base = shapeSpikeFractal(p * 1.2);
    else if (geo == 3) base = shapeSpikeFractal(p * 0.8);
...
vec3 applyTransform(vec3 p, int mode, float chaos, float sym, float chspd) {
    ...
    else if (mode == 3) { p += sin(p * (1.0 + chaos * 2.0) + TIME * chspd) * chaos * 0.5; p = fract(p * 1.5) - 0.75; }
...
vec3 originalLoopColor(vec2 uv, float time) {
    vec3 col = vec3(0.0);
    vec2 cuv = uv;
    for (float i = 0.0; i < 12.0; i++) {
        float tMod = time + i * 1.0;
        cuv = mat2(cos(tMod), -sin(tMod), sin(tMod), cos(tMod)) * cuv;
        cuv = abs(cuv) / (dot(cuv, cuv) + 0.1) - 0.5;
        float d = length(cuv);
        col += vec3(sin(tMod), cos(tMod), sin(tMod * 1.1)) * exp(-d * 7.0);
    }
    return col * (2.0 + 0.8 * sin(time * 0.5));
}
...
void main() {
    ...
    for (int i = 0; i < 48; i++) {
        if (i >= int(StepCount)) break;
        vec3 pOrig = roWarped + dist * rd;

        vec3 pTrans = applyTransform(pOrig, mode, chaos, sym, chspd);
        vec3 pFinal = mix(pOrig, pTrans, TransformMix);

        float d = scene(pFinal, geo, chaos, chaosMix);
        d = max(abs(d), 0.01);

        float fade = exp(-float(i)*0.03*sharp);
        float focus = smoothstep(FocusNear, FocusFar, dist);

        vec3 palCol = getColorPalette(palMode, pFinal.z + t * 0.1);
        vec3 texCol = triplanarTexture(pFinal * TextureScale, 1.0);
    ...
    vec3 baseCol = originalLoopColor(uv, t);
    col = mix(baseCol, col, TransformMix);
