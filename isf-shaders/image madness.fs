/*
{
    "DESCRIPTION": "A synthesis of two shaders, combining 3D raymarching with the logic of a 2D fractal, now with a feedback-based morphing effect.",
    "CATEGORIES": ["Fractal", "Volumetric", "Psychedelic", "Optimized", "ImageFX"],
    "ISFVSN": "2.0",
    "INPUTS": [
        ...
        { "NAME": "FeedbackWarpIntensity", "TYPE": "float", "DEFAULT": 0.1, "MIN": 0.0, "MAX": 1.0 }
    ]
}
*/
...
// Combined scene function
float scene(vec3 p, float t, float geo, float chaos, float mixAmt) {
    float base;
    if (geo < 0.5) base = sdSphere(p, 1.0);
    else if (geo < 1.5) base = sdTorus(p, vec2(1.0, 0.3));
    else if (geo < 2.5) base = shapeSpikeFractal(p);
    else if (geo < 3.5) base = sdGrid(p, 4.0) * 0.1;
    else if (geo < 4.5) base = liminalFractal(p, t);
    else if (geo < 5.5) base = mix(liminalFractal(p, t), sdGrid(p, 4.0) * 0.1, 0.5);
...
void main() {
    // Read the feedback image and use it for a coordinated warp
    vec2 warped_uv = gl_FragCoord.xy / RENDERSIZE.xy;
    vec3 feedback_color = texture2D(feedback, warped_uv).rgb;

    // The feedback color values (0.0 to 1.0) are used to create an offset
    // This creates a "glitch" or "morph" effect where the previous frame
    // distorts the current one.
    vec2 feedback_warp = (feedback_color.rg - 0.5) * FeedbackWarpIntensity;
    
    vec2 uv = (gl_FragCoord.xy + feedback_warp * RENDERSIZE.xy - 0.5 * RENDERSIZE.xy) / RENDERSIZE.y;
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        float d = scene(p, t, geo, chaos, chaosMix);
        d = max(abs(d), 0.01);
