/*
{
    "DESCRIPTION": "A true synthesis of two shaders, blending a 3D raymarcher with the chaotic 2D fractal logic. Now includes more complex fractal geometries.",
    ...
    "INPUTS": [
        ...
        { "NAME": "GeometryType", "TYPE": "float", "DEFAULT": 3, "MIN": 0, "MAX": 8, "VALUES": ["Sphere", "Torus", "Spike Fractal", "Grid Fractal", "Mandelbox", "Mixed Mandelbox", "Mixed Spike", "Menger Sponge", "Mandelbulb"] },
...
// Reused helper functions from the original shader
...
// Advanced palettes from the original shader
...
// Triplanar texturing from the original shader
...
// Mandelbox Base Equation
float shapeMandelbox(vec3 p) {
    vec3 box = p;
    float scale = 2.0;
    for (int i = 0; i < 8; i++) {
        box = clamp(box, -1.0, 1.0) * 2.0 - box;
        float r2 = dot(box, box);
        if (r2 > 4.0) break;
        box = box * scale + p;
    }
    return (length(box) - 1.0) / abs(scale);
}
...
// NEW Geometries: Menger Sponge and Mandelbulb
float shapeMengerSponge(vec3 p) {
    vec3 a = p;
    float d = dot(p, p);
    for (int i = 0; i < 4; i++) {
        a = abs(a);
        a = (a / 3.0) - vec3(1.0/3.0);
        d = min(max(a.x, a.y), max(a.y, a.z));
    }
    return d - 0.2;
}

float shapeMandelbulb(vec3 p) {
    vec3 z = p;
    float dr = 1.0;
    float r = 0.0;
    for (int i=0; i < 8; i++) {
        ...
        z = r8 * vec3(sin(theta)*cos(phi), sin(theta)*sin(phi), cos(theta));
        z += p;
    }
    return 0.5*log(r)*r/dr;
}

// The core scene function with all geometries
float scene(vec3 p, float t, float geo, float chaos, float mixAmt, float LiminalMix, float liminalScale, float fractalMorph) {
    float base;
    ...
    else if (geo < 4.5) base = shapeMandelbox(p);
    else if (geo < 5.5) base = mix(shapeMandelbox(p), sdGrid(p, 4.0) * 0.1, 0.5);
    else if (geo < 6.5) base = mix(shapeSpikeFractal(p), sdSphere(p, 1.0), 0.5);
    else if (geo < 7.5) base = shapeMengerSponge(p);
    else base = shapeMandelbulb(p);

    // This section now correctly blends the base shape with the Mandelbox fractal influence
    float finalShape = mix(base, shapeMandelbox(p), LiminalMix);
    
    // Apply the chaos transformation
    return mix(finalShape, shapeChaos(p, chaos), mixAmt);
}

// Function to apply the chaotic transformations of the 2D shader
...
// Combined Space transformations
...
void main() {
    ...
    // Ray origin and direction, with camera controls
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        float d = scene(p, t, geo, chaos, chaosMix, LiminalMix, LiminalScale, fractalMorph);
        d = max(abs(d), 0.01);
