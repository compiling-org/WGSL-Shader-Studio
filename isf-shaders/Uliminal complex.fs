/*{
  "DESCRIPTION": "Volumetric orbit fractal with chaos, transformation, and morphing geometry.",
  "CATEGORIES": ["Fractal", "Psychedelic", "Dynamic", "Volumetric"],
  "ISFVSN": "2",
  "INPUTS": [
    { "NAME": "Speed", "TYPE": "float", "DEFAULT": 1.5, "MIN": 0.1, "MAX": 5.0 },
    ...
    { "NAME": "IterationsOrbit", "TYPE": "float", "DEFAULT": 6.0, "MIN": 1.0, "MAX": 10.0 },
    { "NAME": "ScaleOrbit", "TYPE": "float", "DEFAULT": 2.5, "MIN": 0.5, "MAX": 5.0 },
    { "NAME": "RotationSpeedOrbit", "TYPE": "float", "DEFAULT": 0.5, "MIN": 0.0, "MAX": 5.0 },
    { "NAME": "GeometryMorph", "TYPE": "float", "DEFAULT": 1.5, "MIN": 0.0, "MAX": 2.0 },
    { "NAME": "PatternType", "TYPE": "float", "DEFAULT": 2.0, "MIN": 0.0, "MAX": 2.0 },
    { "NAME": "ColorModeOrbit", "TYPE": "float", "DEFAULT": 5.0, "MIN": 0.0, "MAX": 6.0 }
  ]
}*/
...
// Orbit fractal color palette
vec3 orbitPalette(float t, int mode) {
  if (mode == 1) return vec3(1.2, 0.3, 0.1) * sin(6.28 * t);
  if (mode == 2) return vec3(0.2, 0.6, 1.0) * cos(6.28 * t);
  if (mode == 3) return vec3(0.5 + 0.5 * sin(t * 10.0 + vec3(0.0, 2.0, 4.0)));
  if (mode == 4) return vec3(0.8 + 0.2 * cos(t * 12.0 + vec3(2.0, 1.0, 4.0))) * vec3(1.2, 0.5, 1.5);
  if (mode == 5) return vec3(sin(8.0 * t + vec3(1.0, 3.0, 5.0)) * 0.5 + 0.5) * vec3(1.5, 1.0, 0.8);
  if (mode == 6) return vec3(0.5 + 0.5 * cos(t * 15.0 + vec3(0.7, 1.4, 2.1)));
  return vec3(0.5 + 0.5 * cos(6.28318 * (vec3(0.0, 0.33, 0.67) * t + vec3(0.153, 0.56, 0.557))));
}
...
vec3 orbitGeometry(vec3 p, float time) {
  vec2 uv = p.xy;
  if (PatternType == 1.0) {
  ...
  for (int i = 0; i < 10; i++) {
    if (i >= int(IterationsOrbit)) break;
    float angle = time * RotationSpeedOrbit + float(i) * 0.5 * length(p.xy) + float(i) * 0.4;
    mat2 rotMat = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
    uv = rotMat * uv;
    uv += reflect(uv, uv);
    uv += vec2(cos(time), sin(time));
    uv = fract(uv * ScaleOrbit) - 0.5;
  }
  return vec3(uv, p.z);
}
...
void main() {
  ...
  for (int i = 0; i < MAX_STEPS; i++) {
    ...
    p = orbitGeometry(p, t); // apply orbit morphing
    ...
    vec3 palCol = orbitPalette(p.z + t * 0.1, int(ColorModeOrbit));
