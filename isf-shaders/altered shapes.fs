...
float noise(vec3 p) {
    ...
    vec2 pos = vec2(uv.x + uv.y*1.0, uv.y+uv.x*1.0);
    return 2.0 * texture(Texture, pos / 1024.0).x - 1.0;
}
...
float addSurfaceDetails(vec3 p, float baseDist, float scale, float bubbleSize, float sporeIntensity) {
    ...
    if (scale > 0.0) {
        ...
        detailDist = min(detailDist, bubbleDist);
        
        // Add spores/grains
        float sporeDist = noise(p * scale * 5.0) * sporeIntensity;
        detailDist = min(detailDist, baseDist + sporeDist);
    }
    return detailDist;
}
...
void main() {
    ...
    for (int i = 0; i < int(stepCount); i++) {
        ...
        // Add recursive surface details
        d = addSurfaceDetails(p, d, surfaceDetailScale, bubbleSize, sporeIntensity);
        
        dist += max(abs(d), 0.01);
        
        float fade = pow(exp(-float(i) * 0.05), falloffCurve);
    ...
    // Psychedelic pulse (color shift and imaginal artifacts)
    float pulse = sin(TIME * ChaosSpeed) * 0.5 + 0.5;
    color *= 1.0 + 0.3 * pulse;

    // Apply texture
    vec2 texCoord = vec2(currentPos.x, currentPos.y) + sin(currentPos.z * TextureWarp) * 0.2;
    vec3 textureColor = texture(Texture, texCoord).rgb;
    color = mix(color, textureColor, 0.5);

    // Apply Depth of Field (DoF)
    float focusDist = hitDistance;
    float blur = 0.0;
    if (focusDist < focusNear) {
        blur = (focusNear - focusDist) / focusNear;
    } else if (focusDist > focusFar) {
        blur = (focusDist - focusFar) / (BAILOUT - focusFar);
    }
    blur = clamp(blur, 0.0, 1.0) * (1.0 - sharpness);
    
    // Simple blur: mix with average color
    vec3 finalColor = mix(color, vec3(0.5), blur);
