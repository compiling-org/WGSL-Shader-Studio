/*{
    ...
    "DESCRIPTION": "A visually aggressive and chaotic raymarcher exploring glitchy geometries, high-contrast colors, and jarring motion to create an abrasive and unsettling aesthetic.",
    "ISFVSN": "2",
    "PASSES": [
        {}
    ],
    "INPUTS": [
        {"NAME": "Speed", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0.1, "MAX": 50},
...
float noise(in vec3 x) {
    ...
    vec2 pos = uv / 1024.0;
    return texture(Texture, pos).x;
}

// A more chaotic, fragmented noise
...
float scene(vec3 p, int geomType, float chaos, float chaosMix, float surfaceDetailScale, float bubbleSize, float sporeIntensity) {
    float baseDist = 0.0;
    if (geomType == 0) baseDist = sdSphere(p, 1.0); // Sphere
    ...
    return addSurfaceDetails(p, mixedDist, surfaceDetailScale, bubbleSize, sporeIntensity);
}
...
void main() {
    ...
    vec3 rd = normalize(vec3(uv * CamFOV, 1.0));

    // Tunnel motion
    if (TunnelMotion > 0.5) {
        ro += vec3(0.0, 0.0, TIME * Speed);
    }
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        vec3 p = ro + rd * dist;
        p = applyTransform(p, int(TransformMode), ChaosIntensity, Symmetry, ChaosSpeed);
        float d = scene(p, int(GeometryType), ChaosIntensity, ChaosMix, SurfaceDetailScale, BubbleSize, SporeIntensity);
    ...
    if (dist < BAILOUT) {
        vec3 p = ro + rd * dist;
        p = applyTransform(p, int(TransformMode), ChaosIntensity, Symmetry, ChaosSpeed);
        vec3 normal = getNormal(p, int(GeometryType), ChaosIntensity, ChaosMix, SurfaceDetailScale, BubbleSize, SporeIntensity);
        vec3 lightDirection = normalize(vec3(LightDirectionX, LightDirectionY, LightDirectionZ));
        float lightIntensity = max(0.0, dot(normal, lightDirection));

        // Get color based on distance and steps
        float t = dist * (1.0 / FalloffCurve) + (steps / StepCount) * 0.1;
        vec3 baseColor = getColorPalette(int(ColorPaletteMode), t);

        // Add lighting
        finalColor = baseColor * lightIntensity;
        
        // Add glow
        finalColor += baseColor * pow(lightIntensity, Glow);

        // Add texture
        vec2 texCoord = p.xy * TextureWarp;
        finalColor += texture(Texture, texCoord).rgb * 0.5;
        ...
        // Background color
        float bg_t = uv.y + uv.x * 0.5;
        finalColor = getColorPalette(int(ColorPaletteMode), bg_t * 0.1);
    }
