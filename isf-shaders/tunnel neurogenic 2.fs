/*{
  "DESCRIPTION": "Tunnel (clean) + Fractal (chaotic) with separate exposures. Background color respected. Transform/Geometry/Chaos affect ONLY the fractal domain, never the tunnel.",
  "CATEGORIES": ["Fractal","Raymarching","Tunnel","Volumetric","Psychedelic"],
  "ISFVSN": "2.0",
  "INPUTS":[
    {"NAME":"masterSpeed","TYPE":"float","DEFAULT":1.0,"MIN":0.1,"MAX":5.0},
    {"NAME":"tunnelExposure","TYPE":"float","DEFAULT":1.0,"MIN":0.0,"MAX":4.0},
    {"NAME":"fractalExposure","TYPE":"float","DEFAULT":1.0,"MIN":0.0,"MAX":4.0},
...
vec3 getColor(float mode, float t){
    if (mode < 8.0) {
        float id = floor(mode);
...
float fractalField(vec3 p){
    ...
    return length(q) / max(abs(w),1.0e-6);
}
...
float shapeSpikeFractal(vec3 p){
    ...
    for(int i=0;i<K;i++){ p=abs(p)/max(dot(p,p),1e-6)-0.5; p*=0.95; d+=length(p); }
    return d*0.05;
}
...
// ---------- transforms (APPLY ONLY TO FRACTAL DOMAIN) ----------
vec3 applyTransformModesFractal(vec3 p,float mode){
...
// ---------- orb path (for tunnel lighting accents) ----------
float orbFunc(vec3 p_in){
    float t_orb = iTime * 2.0;
    vec3 c = vec3(
        ...
        iTime + tan(cos(t_orb * orbTanCosSpeed) * orbTanCosAmplitude) * orbTanCosAmplitude
    );
    return length(p_in - c);
}
...
vec3 applyShimmer(vec3 col,vec2 uv,float t){
    if(shimmerStrength>0.001){
        ...
        vec3 sc = getColor(paletteSelect+0.5, s + t*0.01) * paletteBrightness;
...
vec3 applyColorAdjustments(vec3 col){
    ...
    // vignette
    vec2 uvN = (gl_FragCoord.xy - RENDERSIZE.xy*0.5)/RENDERSIZE.xy;
    float vig = exp(-dot(uvN,uvN)*expFactor);
    col *= mix(1.0, vig, clamp(vignetteStrength*0.5, 0.0, 1.0));
    return col;
}
...
void main(){
    ...
    // Base is ALWAYS background color (fixes color picker)
    vec3 col = backgroundColor.rgb;

    // Screen UV + optional shake
    ...
    // --------- 1) Tunnel march (CLEAN — no transforms/geometry/chaos) ---------
    float d_total=0.0;
    ...
    for(int i=0;i<steps_t;i++){
        ...
        float stepSize = max(raymarchMinDist, abs(sdfTunnel));
        d_total += stepSize;
    }

    // Tunnel shading (only if hit)
    vec3 tunnelCol = vec3(0.0);
    if(tunnelHit){
        vec3 pt = ro + D*d_total;
        vec2 toCenter = pt.xy - P(pt.z).xy;
        float angle = atan(toCenter.y, toCenter.x);
        float radius = length(toCenter);

        // clean tunnel bands + palette
        float band = 0.5 + 0.5*sin(angle*4.0 + pt.z*0.25 + iTime*0.75);
        vec3 palCol = getColor(paletteSelect, iTime*(1.0+paletteAnimSpeed) + pt.z*0.05) * paletteBrightness;

        // flashing lights modulation
        float flash = flashingLightIntensity * (0.5 + 0.5*sin(iTime*flashingLightSpeed));
        float dvec = max(0.0, dot(normalize(vec3(cos(angle), sin(angle), 0.0)), normalize(vec3(1.0,0.0,0.0))));
        float flashMask = mix(1.0, pow(dvec, flashingLightDotVec), flashingLightMixAmount);
        float tunnelLight = band * (1.0 + flash*flashMask);

        // orb-based glows sliding along the tunnel
        float orbD = orbFunc(pt);
        float orbGlow = exp(-orbD*0.4) * 0.35;

        tunnelCol = (palCol * tunnelLight + orbGlow) * tunnelExposure;
    }

    // --------- 2) Fractal/geometry march (CHAOTIC — transforms ON) ---------
    vec3 ro_f = ro + D*d_total;
    float d_f = 0.0;
    vec3 fractalAcc = vec3(0.0);
    ...
    for(int i=0;i<steps_f;i++){
        ...
        // Lock to tunnel frame
        p.xy -= P(p.z).xy;

        // Apply full chaos/transform/geometry only here
        p = applyTransformModesFractal(p, TransformMode);

        float gDist = geometryBase(p);
        float fDist = fractalField(p);
        float dist  = mix(gDist, fDist, 0.5);

        float glow = fractalGlowStrength * exp(-abs(dist)*8.0);
        fractalAcc += glow * globalGlowIntensity;
    ...
    // palette tint for fractal
    vec3 palFr = getColor(paletteSelect, finalPos.z*0.1 + iTime*0.01) * paletteBrightness;
    vec3 fractalCol = fractalAcc * (palFr + 0.5 + 0.5*sin(finalPos)) * fractalExposure;

    // optional texture warp on fractal only
    if((fractalCol.r+fractalCol.g+fractalCol.b) > 1e-5 && TextureWarp>0.001 && TextureScale>0.0){
        vec3 tx = triplanarTexture(finalPos*TextureScale, 1.0);
        fractalCol = mix(fractalCol, tx, clamp(TextureWarp,0.0,1.0)*0.35);
    }

    // pulse lines overlay (both domains)
    if(pulseLineIntensity > 0.0 && (tunnelHit || fractalHit)){
        ...
        vec3 stripeCol = (1.0 - stripe) * pulseLineIntensity * getColor(paletteSelect+0.5, iTime*0.5);
        tunnelCol += stripeCol * 0.5 * tunnelExposure;
        fractalCol += stripeCol * 0.5 * fractalExposure;
    }

    // Compose over background
    vec3 composed = col + tunnelCol + fractalCol;

    // Shimmer & global color adjustments
    composed = applyShimmer(composed, uv, iTime);

    // Soft tonemap to keep super-bright ranges controllable
    composed = 1.0 - exp(-composed);

    composed = applyColorAdjustments(composed);
