/*
{
    "DESCRIPTION": "Fractal shader with controllable tunnel, volumetric spores, inner patterns, and video texture. Includes spore movement, size variation, glow, and pattern pulse.",
...
// SCENE
float scene(vec3 p,float t,float geo,float chaos,float chaosMix,float LiminalMix,float liminalScale){
    float baseShape;
...
void main(){
    ...
    for(int i=0;i<MAX_STEPS;i++){
        ...
        float d=scene(p,t,GeometryType,ChaosIntensity,ChaosMix,LiminalMix,LiminalScale);
        d=max(abs(d),0.01);
