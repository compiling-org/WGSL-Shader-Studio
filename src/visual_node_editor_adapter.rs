...
impl NodeEditorAdapter {
    ...
    pub fn get_node_visual_kind(
        &self,
        _node_graph: &NodeGraph,
        _node_id: NodeId,
    ...
    pub fn ui(&mut self, ui: &mut Ui, node_graph: &mut NodeGraph) {
        ...
        while x < max_x {
            painter.line_segment(
                [
                    pos2(x + self.pan.x, response.rect.top()),
                    pos2(x + self.pan.x, response.rect.bottom()),
                ],
                (1.0, grid_color),
            );
            x += grid_spacing;
        ...
        while y < max_y {
            painter.line_segment(
                [
                    pos2(response.rect.left(), y + self.pan.y),
                    pos2(response.rect.right(), y + self.pan.y),
                ],
                (1.0, grid_color),
            );
            y += grid_spacing;
    ...
    fn draw_placeholder_node(
        ...
        let node_rect = Rect::from_min_size(pos2(position.0, position.1), vec2(120.0, 80.0));
        ...
        painter.circle(
            pos2(node_rect.left() - 8.0, node_rect.center().y),
            6.0,
            Color32::from_gray(100),
            Stroke::new(2.0, Color32::WHITE),
        );
        painter.circle(
            pos2(node_rect.right() + 8.0, node_rect.center().y),
            6.0,
            Color32::from_gray(100),
            Stroke::new(2.0, Color32::WHITE),
        );
