...
"#;
    ...
    println!(
        "   Valid shader diagnostics: {} messages",
        diagnostics.len()
    );
...
"#;
    ...
    println!(
        "   Invalid shader diagnostics: {} messages",
        diagnostics.len()
    );
    ...
    for (i, diagnostic) in diagnostics.iter().enumerate() {
        match diagnostic.severity {
            ...
            DiagnosticSeverity::Warning => {
                println!("   ⚠️  Warning {}: {}", i + 1, diagnostic.message)
            }
...
fn test_timeline_integration() {
    ...
    timeline
        .timeline
        .add_keyframe("time", 0.0, 0.0, InterpolationType::Linear);
    timeline
        .timeline
        .add_keyframe("time", 1.0, 1.0, InterpolationType::Linear);
    timeline
        .timeline
        .add_keyframe("audio_volume", 0.0, 0.5, InterpolationType::Linear);
    timeline
        .timeline
        .add_keyframe("audio_volume", 2.0, 1.0, InterpolationType::EaseInOut);

    println!(
        "   Timeline created with {} tracks",
        timeline.timeline.tracks.len()
    );
    ...
    for time in [0.0, 0.5, 1.0, 1.5, 2.0].iter() {
        ...
        println!(
            "   Time {}: time={:.2}, volume={:.2}",
            time, time_value, volume_value
        );
    }
}
...
fn test_editor_ui_state() {
    ...
    println!(
        "   Parameter 'test_param' set to: {:?}",
        ui_state.get_parameter_value("test_param")
    );
    ...
    ui_state.diagnostics_messages.push(
        resolume_isf_shaders_rust_ffgl::editor_ui::DiagnosticMessage {
    ...
    println!(
        "   Diagnostics messages count: {}",
        ui_state.diagnostics_messages.len()
    );
}
...
"#;
    ...
    let parameters =
        resolume_isf_shaders_rust_ffgl::editor_ui::parse_shader_parameters(test_shader);
