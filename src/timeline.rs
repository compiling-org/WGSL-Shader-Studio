use anyhow::anyhow;
use bevy::prelude::{App, Plugin, Res, ResMut, Resource, Time, Update};
...
impl Timeline {
    ...
    pub fn evaluate(&self, parameter_name: &str, time: f32, default: f32) -> f32 {
        let Some(track) = self.tracks.get(parameter_name) else {
            return default;
        };
        ...
        match (prev, next) {
            (Some((_, p)), Some((_, n))) => {
                ...
                Self::interpolate_values_static(p.value, n.value, t, p.interpolation)
            }
    ...
    pub fn export_to_json(&self) -> Result<String, anyhow::Error> {
        serde_json::to_string_pretty(self)
            .map_err(|e| anyhow!("Failed to serialize timeline: {}", e))
    }
}

/// TimelineAnimation - Bevy resource for timeline animation system
...
impl Plugin for TimelinePlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(TimelineAnimation::default())
            .add_systems(Update, update_timeline_animation);
    }
}

fn update_timeline_animation(mut timeline: ResMut<TimelineAnimation>, time: Res<Time>) {
...
pub fn draw_timeline_ui(ui: &mut Ui, timeline: &mut TimelineAnimation) {
    ui.group(|ui| {
        ui.horizontal(|ui| {
            ...
            ui.label(format!(
                "Time: {:.2}s / {:.2}s",
                timeline.timeline.current_time, timeline.timeline.duration
            ));
            ...
            ui.label("Speed:");
            ui.add(
                bevy_egui::egui::DragValue::new(&mut timeline.timeline.playback_speed)
                    .speed(0.1)
                    .range(0.1..=5.0)
                    .prefix("x"),
            );
        ...
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.allocate_ui(bevy_egui::egui::vec2(timeline_width, ruler_height), |ui| {
                    ...
                    let rect = bevy_egui::egui::Rect::from_min_size(
                        ui.cursor().min,
                        bevy_egui::egui::vec2(timeline_width, ruler_height),
                    );
                    ...
                    let marker_interval = if timeline.timeline.duration > 10.0 {
                        1.0
                    } else {
                        0.5
                    };
                    ...
                    while time <= timeline.timeline.duration {
                        ...
                        if x <= rect.right() {
                            painter.line_segment(
                                [
                                    bevy_egui::egui::pos2(x, rect.top()),
                                    bevy_egui::egui::pos2(x, rect.bottom()),
                                ],
                    ...
                    painter.line_segment(
                        [
                            bevy_egui::egui::pos2(current_x, rect.top()),
                            bevy_egui::egui::pos2(
                                current_x,
                                rect.bottom()
                                    + track_height * timeline.timeline.tracks.len() as f32,
                            ),
                        ],
            ...
            for track_name in &track_names {
                let (track_enabled_snapshot, track_color_snapshot, keyframes_snapshot) =
                    if let Some(track) = timeline.timeline.tracks.get(track_name) {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.allocate_ui(
                        bevy_egui::egui::vec2(timeline_width - 60.0, track_height),
                        |ui| {
                            ui.scope(|ui| {
                                let painter = ui.painter();
                                let rect = bevy_egui::egui::Rect::from_min_size(
                                ...
                                painter.rect_filled(
                                    rect,
                                    bevy_egui::egui::CornerRadius::same(2u8),
                                    bg_color,
                                );

                                // Draw track color indicator (dimmed when disabled)
                                let color_alpha = if track_enabled {
                                    track_color_snapshot[3]
                                } else {
                                    track_color_snapshot[3] * 0.3
                                };
                                ...
                                if track_enabled {
                                    let time_per_pixel =
                                        timeline.timeline.duration / (timeline_width - 60.0);
                                    for keyframe in keyframes_snapshot.iter() {
                                        let x =
                                            rect.left() + 10.0 + (keyframe.time / time_per_pixel);
                                        let y = rect.center().y;
                            ...
                            if track_enabled {
                                ...
                                for keyframe in keyframes_snapshot.iter() {
                                    let x =
                                        ui.cursor().min.x + 10.0 + (keyframe.time / time_per_pixel);
                                    let y = ui.cursor().min.y + track_height / 2.0;

                                    let _keyframe_rect = bevy_egui::egui::Rect::from_center_size(
                                        bevy_egui::egui::pos2(x, y),
        ...
        ui.horizontal(|ui| {
            ...
            if timeline.timeline.loop_enabled {
                ui.label("Start:");
                ui.add(
                    bevy_egui::egui::DragValue::new(&mut timeline.timeline.loop_start)
                        .speed(0.1)
                        .range(0.0..=timeline.timeline.loop_end),
                );
                ui.label("End:");
                ui.add(
                    bevy_egui::egui::DragValue::new(&mut timeline.timeline.loop_end)
                        .speed(0.1)
            ...
            if timeline.timeline.snap_to_grid {
                ui.label("Division:");
                ui.add(
                    bevy_egui::egui::DragValue::new(&mut timeline.timeline.grid_division)
                        .speed(0.01)
                        .range(0.01..=1.0)
                        .suffix("s"),
                );
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Export Timeline").clicked() {
                match timeline.timeline.export_to_json() {
                    Ok(json) => {
                        // Update UI state with exported JSON
                        // This would typically update a field in the UI state to show the exported JSON
                        // or copy to clipboard
                        println!("Timeline exported successfully, length: {}", json.len());
                    }
                    Err(e) => {
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(
            bevy_egui::egui::Slider::new(
                &mut timeline.timeline.current_time,
                0.0..=timeline.timeline.duration,
            )
            .text("Time")
            .suffix("s"),
        );
        ...
        ui.label("Duration:");
        ui.add(
            bevy_egui::egui::DragValue::new(&mut timeline.timeline.duration)
                .speed(0.1)
                .range(1.0..=300.0)
                .suffix("s"),
        );
