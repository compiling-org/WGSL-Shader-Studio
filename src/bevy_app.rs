...
use bevy::prelude::*;
use bevy::render::settings::{Backends, RenderCreation, WgpuFeatures, WgpuLimits, WgpuSettings};
use bevy::render::RenderPlugin;
use bevy::window::{MonitorSelection, WindowPlugin, WindowPosition, WindowResolution};
use bevy_egui::{EguiContexts, EguiPlugin}; // Rely on prelude for ClearColorConfig, Camera2d, Projection, OrthographicProjection
...
use crate::timeline::{PlaybackState, TimelineAnimation};
...
use crate::editor_ui::{draw_editor_code_panel, draw_editor_menu, draw_editor_side_panels};
...
use crate::scene_editor_3d::{EditorManipulable, SceneEditor3DState};
...
pub fn editor_ui_system(
    ...
    mut node_graph_res: Option<
        ResMut<crate::bevy_node_graph_integration_enhanced::NodeGraphResource>,
    >,
    ...
    // [INTEGRATION FIX] Connect Audio Analysis to Shader Parameters
    {
        ...
        crate::editor_ui::connect_audio_to_parameters(
            &mut *ui_state,
    ...
    if timeline_animation.timeline.playback_state == PlaybackState::Playing {
        ...
        for (track_name, track) in &timeline_animation.timeline.tracks {
            if track.enabled && !track.keyframes.is_empty() {
                let value = timeline_animation
                    .timeline
                    .evaluate(track_name, current_time, 0.0);
    ...
    if auditor.enabled && ui_state.show_shader_browser {
        auditor.record_panel("Shader Browser", true, None);
    }
    if auditor.enabled && ui_state.show_parameter_panel {
        auditor.record_panel("Parameters", true, None);
    }
    ...
    if ui_state.show_preview {
        ...
        if auditor.enabled {
            match ui_state.central_view {
                crate::ui::state::CentralView::Preview => {
                    auditor.record_panel("Preview", true, None)
                }
                crate::ui::state::CentralView::NodeGraph => {
                    auditor.record_panel("Node Graph", true, None)
                }
                crate::ui::state::CentralView::Scene3D => {
                    auditor.record_panel("3D Editor", true, None)
                }
...
fn async_initialize_wgpu_renderer(
    ...
    static RENDER_CHANNEL: Lazy<(
        mpsc::Sender<RenderResult>,
        Mutex<mpsc::Receiver<RenderResult>>,
    )> = Lazy::new(|| {
    ...
    static INIT_STARTED: Lazy<std::sync::atomic::AtomicBool> =
        ...
        std::thread::spawn(move || {
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                pollster::block_on(async {
                    super::shader_renderer::ShaderRenderer::new_with_size((800, 600))
                        .await
            ...
            match result {
                ...
                Err(_) => {
                    let _ = tx.send(Err("WGPU initialization panicked".to_string()));
                }
            }
        });
    }
...
fn blocking_initialize_wgpu_renderer(mut ui_state: ResMut<EditorUiState>) {
    ...
    let mut init_err: Option<String> = None;
    {
        ...
        if lock.is_none() {
            ...
            let result = pollster::block_on(super::shader_renderer::ShaderRenderer::new_with_size(
                (800, 600),
            ))
            .map_err(|e| e.to_string());
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    match pollster::block_on(super::shader_renderer::ShaderRenderer::new_with_size(
                        (512, 512),
                    )) {
...
pub fn run_app() {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        let _ = std::fs::write(
            "panic_log.txt",
            format!("Panic occurred at {}:\n{}\n", chrono::Local::now(), msg),
        );
        if msg.contains("wgpu error: Validation Error")
            || msg.contains("Encoder is invalid")
            || msg.contains("SurfaceAcquireSemaphores")
        {
            eprintln!(
                "Caught wgpu validation error (known Bevy 0.17 + bevy_egui issue): {}",
                info
            );
    ...
    app.insert_resource(EditorUiState::default())
        ...
        .insert_resource(VisualNodeEditorState {
            auto_compile: true,
