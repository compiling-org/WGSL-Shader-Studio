use super::state::{CodeEditorTab, DiagnosticSeverity, EditorUiState};
use bevy_egui::egui;
use std::sync::Arc;
            ...
            match ui_state.code_editor_tab {
                CodeEditorTab::Editor => {
                    ...
                    if ui.button("Apply to Preview").clicked() {
                        // Trigger rebuild by updating cache key
                        ui_state.cache_key.3 = ui_state.parameter_values_hash;
                    }
                ...
                CodeEditorTab::AI => {
                    ...
                    if ui.button("Generate Shader").clicked() {
                        ui_state.wgsl_smith_generated =
                            crate::editor_ui::generate_shader_with_wgsl_smith(
                                &ui_state.wgsl_smith_prompt,
                            );
                    }
                }
                CodeEditorTab::Diagnostics => {
                    ...
                    if ui.button("Check Current Shader").clicked() {
                        ui_state.diagnostics_messages =
                            crate::editor_ui::check_wgsl_diagnostics(&ui_state.draft_code);
                    }
