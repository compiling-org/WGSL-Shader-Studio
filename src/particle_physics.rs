...
impl Plugin for ParticlePhysicsPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ParticlePhysicsSettings>()
...
fn spawn_particles(
    ...
    _settings: Res<ParticlePhysicsSettings>,
    time: Res<Time>,
) {
    let delta_time = time.delta_secs();

    for (_entity, mut system) in particle_systems.iter_mut() {
...
pub fn create_particle_system(commands: &mut Commands, position: Vec3, spawn_rate: f32) -> Entity {
    commands
        .spawn((
            ParticleSystem {
                spawn_rate,
...
pub enum ParticleForce {
    ...
    Interaction { strength: f32, radius: f32 },
}
