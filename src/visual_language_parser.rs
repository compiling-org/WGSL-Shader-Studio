...
impl VisualLanguageParser {
    ...
    fn parse_uniform_access(
        ...
        if line.contains("uniforms.time") {
            let _node_id = node_graph.add_node(NodeKind::Time, "Time", (0.0, 0.0));
        } else if line.contains("uniforms.resolution") {
            let _node_id = node_graph.add_node(NodeKind::Resolution, "Resolution", (0.0, 0.0));
        }
    ...
    fn parse_texture_sample(
        ...
        let _node_id = node_graph.add_node(NodeKind::TextureSample, "TextureSample", (0.0, 0.0));
        Ok(())
    }
    ...
    fn parse_return_statement(
        ...
        let _return_expr = &line[7..line.len() - 1].trim(); // Skip "return " and ";"

        // Create an output node
        let _node_id = node_graph.add_node(NodeKind::OutputColor, "Output", (200.0, 0.0));
        Ok(())
    }
    ...
    fn parse_function_call(
        ...
        if line.contains("sin(") {
            let _node_id = node_graph.add_node(NodeKind::Sine, "Sine", (0.0, 0.0));
        } else if line.contains("cos(") {
            let _node_id = node_graph.add_node(NodeKind::Cosine, "Cosine", (0.0, 0.0));
        } else if line.contains("mix(") {
            let _node_id = node_graph.add_node(NodeKind::Mix, "Mix", (0.0, 0.0));
        } else if line.contains("abs(") {
            let _node_id = node_graph.add_node(NodeKind::Abs, "Abs", (0.0, 0.0));
        } else if line.contains("length(") {
            let _node_id = node_graph.add_node(NodeKind::Length, "Length", (0.0, 0.0));
        } else if line.contains("normalize(") {
            let _node_id = node_graph.add_node(NodeKind::Normalize, "Normalize", (0.0, 0.0));
        } else if line.contains("dot(") {
            let _node_id = node_graph.add_node(NodeKind::Dot, "Dot", (0.0, 0.0));
        } else if line.contains("cross(") {
            let _node_id = node_graph.add_node(NodeKind::Cross, "Cross", (0.0, 0.0));
        } else if line.contains("min(") {
            let _node_id = node_graph.add_node(NodeKind::Min, "Min", (0.0, 0.0));
        } else if line.contains("max(") {
            let _node_id = node_graph.add_node(NodeKind::Max, "Max", (0.0, 0.0));
        } else if line.contains("pow(") {
            let _node_id = node_graph.add_node(NodeKind::Pow, "Pow", (0.0, 0.0));
        } else if line.contains("sqrt(") {
            let _node_id = node_graph.add_node(NodeKind::Sqrt, "Sqrt", (0.0, 0.0));
        } else if line.contains("floor(") {
            let _node_id = node_graph.add_node(NodeKind::Floor, "Floor", (0.0, 0.0));
        } else if line.contains("ceil(") {
            let _node_id = node_graph.add_node(NodeKind::Ceil, "Ceil", (0.0, 0.0));
        } else if line.contains("fract(") {
            let _node_id = node_graph.add_node(NodeKind::Fract, "Fract", (0.0, 0.0));
        } else if line.contains("sign(") {
            let _node_id = node_graph.add_node(NodeKind::Sign, "Sign", (0.0, 0.0));
        } else if line.contains("step(") {
            let _node_id = node_graph.add_node(NodeKind::Step, "Step", (0.0, 0.0));
        } else if line.contains("smoothstep(") {
            let _node_id = node_graph.add_node(NodeKind::Smoothstep, "Smoothstep", (0.0, 0.0));
        } else if line.contains("clamp(") {
            let _node_id = node_graph.add_node(NodeKind::Clamp, "Clamp", (0.0, 0.0));
        }
...
"#;
        ...
        assert!(graph
            .nodes
            .values()
            .any(|n| matches!(n.kind, NodeKind::Time)));
        assert!(graph
            .nodes
            .values()
            .any(|n| matches!(n.kind, NodeKind::OutputColor)));
        assert!(graph
            .nodes
            .values()
            .any(|n| matches!(n.kind, NodeKind::Sine)));
    }
