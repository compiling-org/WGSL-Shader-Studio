use super::diagnostics::{Diagnostic, DiagnosticSeverity, Diagnostics};
use anyhow::Result;
...
impl WgslEmitter {
        ...
        for (group, uniforms) in sorted_groups {
            ...
            for (i, uniform) in sorted_uniforms.iter().enumerate() {
                ...
                self.output.push_str(&format!(
                    "@binding({}) var<uniform> {}: {}",
                    uniform.binding, uniform.name, uniform.wgsl_type
                ));
            }
    ...
    fn emit_global_vars(&mut self, global_vars: &[GlobalVarInfo]) -> Result<()> {
        ...
        for var in global_vars {
            self.output.push_str(&format!(
                "var<{}> {}: {};\n",
                var.storage_class, var.name, var.wgsl_type
            ));
        }
    ...
    fn emit_function(&mut self, function: &FunctionInfo) -> Result<()> {
        ...
        for (i, param) in function.parameters.iter().enumerate() {
            ...
            self.output
                .push_str(&format!("{}: {}", param.name, param.wgsl_type));
        }
    ...
    fn emit_main_function(&mut self, shader_type: ShaderType, main_body: &str) -> Result<()> {
        match shader_type {
            ShaderType::Vertex => {
                ...
                self.output
                    .push_str("    @location(0) position: vec3<f32>,\n");
                self.output.push_str("    @location(1) uv: vec2<f32>,\n");
                self.output
                    .push_str(") -> @builtin(position) vec4<f32> {\n");
            ...
            ShaderType::Fragment => {
                ...
                self.output.push_str(&"    ".repeat(self.indent_level));
                self.output
                    .push_str("return vec4<f32>(1.0, 0.0, 0.0, 1.0); // Default red color\n");
                self.indent_level -= 1;
...
mod tests {
    ...
    fn test_glsl_type_conversion() {
        ...
        assert_eq!(
            WgslEmitter::glsl_type_to_wgsl("sampler2D"),
            "texture_2d<f32>"
        );
    }
    ...
    fn test_hlsl_type_conversion() {
        ...
        assert_eq!(
            WgslEmitter::hlsl_type_to_wgsl("Texture2D"),
            "texture_2d<f32>"
        );
    }
