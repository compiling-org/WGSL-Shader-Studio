...
impl HLSLConverter {
    ...
    fn generate_simple_wgsl(&mut self, _hlsl_source: &str, file_path: &str) -> Result<String> {
    ...
    fn generate_constant_buffers(&mut self, wgsl_code: &mut String) -> Result<()> {
        for (_index, cbuffer) in self.constant_buffers.iter().enumerate() {
    ...
    fn generate_texture_declarations(&mut self, wgsl_code: &mut String) -> Result<()> {
        for (_index, texture) in self.texture_declarations.iter().enumerate() {
            wgsl_code.push_str(&format!(
                "@group({}) @binding({}) var {}: {};\n",
