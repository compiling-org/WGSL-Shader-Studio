...
impl VisualLanguageManager {
    ...
    pub fn round_trip_conversion(
        &mut self,
        original_graph: &NodeGraph,
    ) -> Result<NodeGraph, Vec<String>> {
        self.bridge.round_trip_conversion(original_graph)
    }

    /// Synchronize a visual node editor with WGSL code
    pub fn sync_editor_with_code(
        &mut self,
        _editor: &mut EnhancedVisualNodeEditor,
        wgsl_code: &str,
    ) -> Result<(), Vec<String>> {
        let _graph = self.parse_to_graph(wgsl_code)?;
    ...
    pub fn validate_graph(&mut self, node_graph: &NodeGraph) -> Result<(), Vec<String>> {
        ...
        match self.bridge.is_equivalent_after_round_trip(node_graph) {
            Ok(true) => Ok(()),
            Ok(false) => Err(vec![
                "Graph is not equivalent after round-trip conversion".to_string()
            ]),
            Err(errors) => Err(errors),
        }
    }
    ...
    pub fn optimize_graph(&self, node_graph: &mut NodeGraph) {
        ...
        for (node_id, node) in &node_graph.nodes {
            ...
            let has_outputs = node_graph
                .connections
                .iter()
                .any(|conn| conn.from_node == *node_id);
    ...
    fn is_node_connected_to_output(
        &self,
        node_id: crate::node_graph::NodeId,
        node_graph: &NodeGraph,
    ) -> bool {
        // Find all output nodes
        let output_nodes: Vec<_> = node_graph
            .nodes
            .iter()
    ...
    fn can_reach_node(
        &self,
        source_node: crate::node_graph::NodeId,
        target_node: crate::node_graph::NodeId,
        node_graph: &NodeGraph,
    ) -> bool {
