...
impl ComputePassManager {
    ...
    pub fn create_ping_pong_texture(
        &mut self,
        name: &str,
        width: u32,
        height: u32,
        ...
        self.ping_pong_textures
            .insert(name.to_string(), ping_pong_texture);
    }
    ...
    pub fn create_compute_pipeline(
        ...
        shader_code: String,
        bind_group_layouts: Vec<BindGroupLayoutResource>,
    ) {
        ...
        self.compute_pipelines
            .insert(name.to_string(), compute_pipeline);
    }

    /// Create a compute pass execution
    pub fn create_compute_pass_execution(
        &mut self,
        name: &str,
        pipeline_name: &str,
    ...
    pub fn create_ping_pong_buffer(&mut self, name: &str, size: usize) {
        ...
        self.ping_pong_buffers
            .insert(name.to_string(), ping_pong_buffer);
    }
    ...
    pub fn create_storage_texture(
        ...
        self.storage_textures
            .insert(name.to_string(), storage_texture);
    }
    ...
    pub fn generate_particle_compute_wgsl(&self) -> String {
        ...
        wgsl.push_str(
            "fn particle_simulate(@builtin(global_invocation_id) global_id: vec3<u32>) {\n",
        );
        ...
        wgsl.push_str(
            "    particle.velocity.y = particle.velocity.y - 9.8 * uniforms.delta_time;\n",
        );
        wgsl.push_str("    \n");
        ...
        wgsl.push_str(
            "    if (particle.position.x < 0.0 || particle.position.x > uniforms.resolution.x) {\n",
        );
        wgsl.push_str("        particle.velocity.x = -particle.velocity.x * 0.8;\n");
        wgsl.push_str("    }\n");
        wgsl.push_str(
            "    if (particle.position.y < 0.0 || particle.position.y > uniforms.resolution.y) {\n",
        );
        ...
        wgsl.push_str(
            "        particle.velocity = vec2<f32>((f32(index) - 32.0) * 10.0, -50.0);\n",
        );
        ...
        wgsl.push_str(
            "    textureStore(output_texture, coord, vec4<f32>(1.0, 0.5, 0.0, alpha));\n",
        );
        wgsl.push_str("}\n");
    ...
    pub fn generate_game_of_life_wgsl(&self) -> String {
        ...
        wgsl.push_str(
            "@group(1) @binding(0) var current_state: texture_storage_2d<rgba8unorm, read>;\n",
        );
        wgsl.push_str(
            "@group(1) @binding(1) var next_state: texture_storage_2d<rgba8unorm, write>;\n\n",
        );
        ...
        wgsl.push_str(
            "                let neighbor_color = textureLoad(current_state, neighbor_coord);\n",
        );
...
fn update_compute_passes(mut compute_manager: ResMut<ComputePassManager>, time: Res<Time>) {
    // Update ping-pong resources every frame
    for texture in compute_manager.ping_pong_textures.values_mut() {
        if time.elapsed_secs() > 0.016 {
            // 60 FPS
            texture.frame_count += 1;
        }
    }
