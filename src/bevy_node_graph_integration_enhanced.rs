...
impl ShaderNodeGraph {
    ...
    pub fn create_default_shader_graph(&mut self) {
        ...
        // Add resolution node
        let _resolution_node = self.add_node(
    ...
    pub fn generate_wgsl(&self) -> Result<String, String> {
        ...
        for (_var_name, var_code) in node_vars {
...
fn update_node_graph(
    ...
    // Sync all node parameters to UI parameter map for renderer
    for node in node_graph.graph.nodes.values() {
        for (param_name, value) in &node.parameters {
            ui_state.set_parameter_value(param_name, *value);
        }
    }
}

/// Draw the main node graph UI
