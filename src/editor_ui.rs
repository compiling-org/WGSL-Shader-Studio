...
pub fn draw_preview_area(
    ...
    _spout_output: &mut SpoutSyphonOutput,
    _ndi_output: &mut NdiOutput,
...
fn compile_and_render_shader(
    ...
    let needs_refresh = ui_state.cache_key != cache_key;
    if !needs_refresh {
        if let Some(ref texture) = ui_state.preview_texture_handle {
            return Ok(texture.clone());
        }
...
pub fn draw_editor_menu(
    ctx: &egui::Context,
    ui_state: &mut EditorUiState,
    _auditor: &mut crate::simple_ui_auditor::SimpleUiAuditor,
) {
    egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
        egui::MenuBar::new().ui(ui, |ui| {
                ...
                ui.menu_button("View", |ui| {
                    if ui.button("Shader Browser").clicked() { ui_state.show_shader_browser = !ui_state.show_shader_browser; ui.close_kind(egui::UiKind::Menu); }
                    if ui.button("Parameters").clicked() { ui_state.show_parameter_panel = !ui_state.show_parameter_panel; ui.close_kind(egui::UiKind::Menu); }
                    if ui.button("Preview").clicked() { ui_state.show_preview = !ui_state.show_preview; ui.close_kind(egui::UiKind::Menu); }
                    if ui.button("Code Editor").clicked() { ui_state.show_code_editor = !ui_state.show_code_editor; ui.close_kind(egui::UiKind::Menu); }
                    if ui.button("Diagnostics").clicked() { ui_state.show_diagnostics_panel = !ui_state.show_diagnostics_panel; ui.close_kind(egui::UiKind::Menu); }
                    if ui.button("Node Graph").clicked() { ui_state.central_view = CentralView::NodeGraph; ui.close_kind(egui::UiKind::Menu); }
                });
